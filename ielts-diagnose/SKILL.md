...
**根据用户的备考总时长，按以下比例分配各阶段周数：**

| 阶段 | 比例 | 重点 |
|------|------|------|
| 摸底 | 10% | 做1套真题定位 + 看Simon写作课 + 下载口语题库 |
| 建基础 | 25% | 背单词（/ielts-vocab） + 语料库 + 口语5个万能故事 |
| 分科强化 | 40% | 每天按时间分配执行 + 周末模考 |
| 冲刺 | 15% | 60%时间给弱项 + 周末模考 |
| 考试模式 | 10% | 隔天全真模考 + 只巩固不学新 |

**计算规则：** 根据用户的备考总时长，按以上比例分配各阶段周数。不足1周的阶段合并到相邻阶段。
...
|------|-----|-----|-----|------|
| 摸底 | — | 第1周(前半) | 第1周 | 第1周 |
| 建基础 | 第1周 | 第1周(后半)-第2周 | 第2周 | 第2-4周 |
| 分科强化 | 第2周 | 第3-4周 | 第3-5周 | 第5-9周 |
| 冲刺 | 第3周 | 第5周 | 第6-7周 | 第10-11周 |
| 考试模式 | 第4周 | 第6周 | 第8周 | 第12周 |

**极短备考（<4周）最小可行计划：** 跳过摸底阶段（假设用户已有成绩数据），压缩建基础为2-3天，全部时间投入分科强化和模考交替。每天至少1套完整模考或2个单科模考。
